...
class Ray:
    ...
    def drawRays(self, walls):
        ...
        self.line = pygame.draw.line(self.screen, colors.WHITE(), (self.x1, self.y1), (self.x2,self.y2), 5)
        for wall in walls:
            if self.line.colliderect(wall[0]):
                ...
                den = (self.x1 - self.x2) * (self.y3-self.y4) - (self.y1 - self.y2) * (self.x3 - self.x4)
                if den == 0:
                    print('there is a issue')
                t = (self.x1 - self.x3)*(self.y3-self.y4)-(self.y1-self.y3)*(self.x3-self.x4) /den
                u = -(self.x1 - self.x3) * (self.y1-self.y2) - (self.y1-self.y3)*(self.x1-self.x2) / den
